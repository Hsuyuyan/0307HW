...
## Supervised learning 監督式學習 -1 有答案 --錄取不錄取 有或沒有
- Regression 迴歸
...
- Clustering 分群
    ...
    - K-means clustering `kmeans()` #距離接近的圈一起
...
## Apriori演算法 p17 a=牛奶 b=青菜 ab憶起出現的機率為?
<img src="Fig/apriori.png" width="700px">

## 超市資料分析：讀取資料(橫 一筆交易  行 各種物品)
...
rulesR<-apriori(data=Groceries, parameter=list(supp=0.001,conf = 0.08), 
        appearance = list(default="lhs",rhs="whole milk"), #rhs=right hand side #設定右邊一定要是牛奶
        control = list(verbose=F)) #不要顯示output
...
- 通常會比例分配
    - 2/3的資料設為`Training set` 校內模擬考 期中期末考(學會)
    - 剩下的1/3做驗證`Test set`學測 指考(驗證學得如何)
      `Training set``Test set`需分開
...
sample(1:nrow(NBA1516),nrow(NBA1516)/3) #從第一行到最後一行(假如900人)，隨機取1/3行數(取300人)
```

## 把NBA的資料分成Training 和 Test set (新增一個欄位 代表此列是否是test 是 為true)
```{r message=FALSE,warning=FALSE}
...
## 用Test set來評估模型好不好 predictPoint預測
...
mydata$admit<-factor(mydata$admit,levels=c(1,0))#若是c(0,1)代表Level 1正面為不錄取
```
...
head(AdmitProb)
table(AdmitProb<0.5,mydata[mydata$Test==T,]$admit) # row,column false且0 若模型說會被錄取 而真被錄取 14人
```
